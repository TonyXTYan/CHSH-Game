...
### Phased Implementation (3 Steps)

Step 1 — Detection, Dashboard surfacing, and Non-cheat safety
- Scope:
  - Implement robust `cheat_type` parsing; store only `cheat_type` in `state.active_teams`.
  - Expose `cheat` and `cheat_type` from `get_all_teams()`; render 🤡 and type tag in dashboard.
  - Add dashboard toggle to ban cheats (kick current cheaters, block new `cheat-*`).
  - Ensure non-cheat teams are unaffected (no new events or relaxations).
- Testing:
  - Unit: `parse_cheat_type` (regex + error paths).
  - Integration: dashboard shows 🤡 and `[com|hint|tony|kevin]`; ban toggle kicks cheaters and blocks new joins; honest teams receive no `cheat:*` events.
- Milestones:
  - Dashboard shows cheat icons/tags; ban toggle works; no behavior changes for honest teams.

Step 2 — Communication and Hints (com, hint)
- Scope:
  - `cheat-com`: on submit, emit `cheat:partner_choice` to teammate.
  - `cheat-hint`: emit at most twice per round (round start; after first answer); show recommended button with rainbow glow; include parity reason. Disable hints in AQM Joe v1 (acts like com only).
  - Performance: cache parity per round, avoid recompute.
- Testing:
  - Integration: com emits partner choice; hint emits at round start and updates after first answer; AQM Joe mode emits no hints.
  - Non-cheat isolation tests continue to pass.
- Milestones:
  - Players in com/hint teams observe correct partner-choice/hints; no regressions for honest teams.

Step 3 — Single-player auto-complete (tony, kevin) with DB consistency
- Scope:
  - Relax `on_submit_answer` and allow `start_new_round_for_pair` for single-player `tony/kevin` teams.
  - Auto-fill partner answer transactionally only when exactly one player is connected at submit time (win for tony, lose for kevin); stop auto-fill if second player is present.
...
- Testing:
  ...
  - Non-cheat isolation re-verified.
- Milestones:
  - Stable single-player flow with correct DB writes (max two Answers per round) and reliable round progression.
...
- File: `src/state.py`
  - Add `cheats_banned: bool` (default False) to global state.
...
### Acceptance Criteria
- Teams with names starting `cheat-com*` show partner’s choice to the teammate upon submit.
- Teams with names starting `cheat-hint*` receive hints at round start and update after first answer; UI shows glow and reasoning.
- `cheat-tony*` single-player teams auto-complete partner answer to win consistent with player’s choice.
- `cheat-kevin*` single-player teams auto-complete partner answer to lose consistent with player’s choice.
- Dashboard can ban cheats; existing cheating teams are kicked and new `cheat-*` names are rejected. Dashboard shows 🤡 for cheating teams.
 - Incomplete `cheat-tony`/`cheat-kevin` teams can submit answers (client buttons enabled; server accepts submission) and rounds auto-complete.
 - In AQM Joe mode, `cheat-hint` teams do not receive hints (partner-choice only), unless AQM Joe–specific hinting is implemented.
 - Dashboard rows receive and render `cheat_type` from `get_all_teams()` and display the 🤡 and a type tag.
 - New unit and integration tests pass under pytest, following the existing test harness patterns.
