...
def get_effective_combo_repeats(game_mode=None):
    ...
    Returns:
        int: TARGET_COMBO_REPEATS*2 for 'new'/'simplified' mode, TARGET_COMBO_REPEATS for others
    """
    if game_mode is None:
        from src.state import state
        game_mode = state.game_mode
    
    if game_mode in ('new', 'simplified'):
...
def start_new_round_for_pair(team_name):
    try:
        ...
        if state.game_mode in ('new', 'simplified'):
            # Simplified mode: Player 1 gets A,B only; Player 2 gets X,Y only
            ...
            # Classic and AQM Joe modes: All players can get any question type
