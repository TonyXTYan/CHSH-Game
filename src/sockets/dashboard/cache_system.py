"""
Cache system for dashboard computations.

Provides SelectiveCache class and decorators for efficient caching with selective invalidation.
"""
...
def clear_team_caches() -> None:
    """
    Clear cached data for all teams but preserve client activity tracking.
    Used when teams are deleted/modified and we need fresh calculations.
    """
    try:
        with _safe_dashboard_operation():
            # Clear selective caches only
            ...
            # Reset global throttling variables
            ...
            logger.info("Cleared all team caches and throttling state")
    except Exception as e:
        logger.error(f"Error clearing team caches: {str(e)}", exc_info=True)

def force_clear_all_caches() -> None:
    """
    Forcefully clear all caches and reset all state for major operations.
    Used when game mode changes or during full game resets.
    """
    try:
        with _safe_dashboard_operation():
            # Clear all caches
            ...
            logger.info("Force cleared all caches")
    except Exception as e:
        logger.error(f"Error in force clear all caches: {str(e)}", exc_info=True)

def invalidate_team_caches(team_name: str) -> None:
    """
    Invalidate cached data for a specific team across all cache types.
    Used when a specific team's data changes to avoid full cache clear.
    """
    try:
        with _safe_dashboard_operation():
            total_invalidated = 0
            ...
            total_invalidated += _hash_cache.invalidate_by_team(team_name)
            total_invalidated += _correlation_cache.invalidate_by_team(team_name)
            total_invalidated += _success_cache.invalidate_by_team(team_name)
            total_invalidated += _classic_stats_cache.invalidate_by_team(team_name)
            total_invalidated += _new_stats_cache.invalidate_by_team(team_name)
            total_invalidated += _team_process_cache.invalidate_by_team(team_name)
            
            # Reset global throttling to force refresh
            global _last_refresh_time, _cached_teams_result
            global _last_team_update_time, _last_full_update_time
            global _cached_team_metrics, _cached_full_metrics
