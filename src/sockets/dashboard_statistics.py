"""
Statistical computation functions for the dashboard.

This module handles correlation matrices, success metrics, and all statistical
calculations for both classic and new game modes.
"""
...
from .dashboard_cache import (
    ...
    _classic_stats_cache, _new_stats_cache
)

# Configure logging
logger = logging.getLogger(__name__)

# Minimum standard deviation to avoid zero uncertainty warnings
MIN_STD_DEV = 1e-10
...
def compute_team_hashes(team_name: str) -> Tuple[str, str]:
    ...
    try:
        from src.models.quiz_models import PairQuestionRounds, Answers
...
def compute_correlation_matrix(team_name: str) -> Tuple[List[List[Tuple[int, int]]], List[str], float, Dict[str, float], Dict[str, Dict[str, int]], Dict[Tuple[str, str], int], Dict[Tuple[str, str], int]]:
    """
    Compute correlation matrix for classic mode from team rounds and answers.
    Returns correlation matrix, item values, balance metrics, and count data.
    """
    try:
        from src.models.quiz_models import PairQuestionRounds, Answers
