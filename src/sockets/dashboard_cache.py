"""
Cache management system for dashboard functionality.

This module provides selective caching capabilities that support team-specific
cache invalidation while maintaining high performance through LRU caching.
"""
...
def invalidate_team_caches(team_name: str) -> None:
    """
    Invalidate all team-specific cache entries across all cache instances.
    This function selectively clears only the data for the specified team,
    preserving cached results for other teams.
    
    Args:
        team_name: Name of the team whose cache entries should be invalidated
    """
    try:
        logger.debug(f"Invalidating caches for team: {team_name}")
        
        # Track total invalidations for logging
        total_invalidated = 0
        
        # Invalidate team-specific entries in all caches
        cache_instances = [
            (_hash_cache, "hash"),
            (_correlation_cache, "correlation"), 
            (_success_cache, "success"),
            (_classic_stats_cache, "classic_stats"),
            (_new_stats_cache, "new_stats"),
            (_team_process_cache, "team_process")
        ]
        
        for cache_instance, cache_name in cache_instances:
            invalidated = cache_instance.invalidate_by_team(team_name)
            total_invalidated += invalidated
            if invalidated > 0:
                logger.debug(f"Invalidated {invalidated} entries from {cache_name} cache for team {team_name}")
        
        if total_invalidated > 0:
            logger.info(f"Selectively invalidated {total_invalidated} cache entries for team {team_name}")
        else:
            logger.debug(f"No cache entries found for team {team_name}")
...
def clear_team_caches() -> None:
    """
    Clear all team-related caches and reset global throttling state.
    This is a more aggressive operation that clears all cached data.
    """
    try:
        logger.info("Clearing all team caches")
        ...
        dashboard_utils.reset_throttling_state()
        
        logger.info("All team caches cleared successfully")
...
def force_clear_all_caches() -> None:
    """
    Force clear all caches and reset all throttling state.
    Used when game mode changes or other global state changes occur.
    This is the most aggressive cache clearing operation.
    """
    try:
        logger.info("Force clearing all caches (including dashboard client tracking)")
        
        # Clear all team caches
        clear_team_caches()
        
        # Clear dashboard client tracking caches
        from . import dashboard_handlers
        dashboard_handlers.reset_client_tracking()
        
        logger.info("All caches force-cleared successfully")
        
    except Exception as e:
        logger.error(f"Error force clearing caches: {str(e)}", exc_info=True)

# Export cache instances for use by other modules
__all__ = [
    'SelectiveCache',
    'selective_cache',
    '_hash_cache',
