...
from src.sockets.dashboard import emit_dashboard_team_update, emit_dashboard_full_update, invalidate_teams_list_cache
            ...
            if team_info:
                ...
                if db_team:
                    ...
                    if sid in team_info['players']:
                        ...
                        # Invalidate teams list cache since team status changed (player disconnected)
...
def on_create_team(data):
    try:
        ...
        # Invalidate teams list cache since a new team was created
        invalidate_teams_list_cache()
...
def on_join_team(data):
    try:
        ...
        if db_team:
            ...
            # Invalidate teams list cache since team status changed (player join/leave)
            invalidate_teams_list_cache()
...
def on_reactivate_team(data):
    try:
        ...
        with app.app_context():
            ...
            # Invalidate teams list cache since team was reactivated
            invalidate_teams_list_cache()
    ...
    try:
        ...
        if sid in team_info['players']:
            ...
            if db_team: # Commit changes if db_team was involved
                ...
                # Invalidate teams list cache since team status changed (player left)
                invalidate_teams_list_cache()
