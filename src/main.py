...
app.config['SQLALCHEMY_DATABASE_URI'] = os.environ.get('DATABASE_URL') or 'sqlite:///' + os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', 'quiz_app.db')
app.config['SQLALCHEMY_TRACK_MODIFICATIONS'] = False
db.init_app(app)

socketio = SocketIO(app, cors_allowed_origins="*")

active_teams = {}  # {team_name: {'creator_sid': sid, 'participant2_sid': None, 'team_id': db_team_id, 'current_round_number': 0, 'combo_tracker': {}, 'current_db_round_id': None, 'p1_answered_current_round': False, 'p2_answered_current_round': False}}
participant_to_team = {}  # {sid: team_name}
dashboard_clients = set() # Stores SIDs of connected dashboard clients
...
def get_serialized_active_teams():
    teams_list = []
    for name, info in active_teams.items():
        teams_list.append({
            'team_name': name,
...
def emit_dashboard_full_update(client_sid=None):
    # Sends a more comprehensive update, potentially including recent answers or total counts
    # For now, it's similar to team update but could be expanded
    with app.app_context(): # Ensure DB access is within app context
        total_answers = Answers.query.count()
    update_data = {
        'active_teams': get_serialized_active_teams(),
        'total_answers_count': total_answers 
        # 'recent_answers': [] # Could add logic to fetch recent N answers if needed
    }
    if client_sid:
        socketio.emit('dashboard_update', update_data, room=client_sid)
    else:
        for dash_sid in dashboard_clients:
...
def on_dashboard_join():
    ...
    emit_dashboard_full_update(client_sid=sid) # Send initial full update to the new dashboard client

@socketio.on('connect')
def handle_connect():
    print(f'Client connected: {request.sid}')
    # Do not add to dashboard_clients here, only on 'dashboard_join'
...
def handle_disconnect():
    ...
    if sid in dashboard_clients:
        dashboard_clients.remove(sid)
        print(f"Dashboard client disconnected: {sid}")

    if sid in participant_to_team:
        team_name = participant_to_team[sid]
        team_info = active_teams.get(team_name)
        if team_info:
            ...
            if db_team:
                ...
                if creator_left:
                    if team_info.get('participant2_sid'):
                        ...
                        del participant_to_team[team_info['participant2_sid']]
                        leave_room(team_name, sid=team_info['participant2_sid'])
                    del active_teams[team_name]
                ...
                if creator_left or partner_left:
                    ...
                    if not creator_left and team_info.get('creator_sid'): # if partner left and creator still there
...
def on_create_team(data):
    ...
    if team_name in active_teams or Teams.query.filter_by(team_name=team_name, is_active=True).first():
    ...
    active_teams[team_name] = {'creator_sid': sid, 'participant2_sid': None, 'team_id': new_team_db.team_id, 'current_round_number': 0, 'combo_tracker': {}, 'p1_answered_current_round': False, 'p2_answered_current_round': False}
    participant_to_team[sid] = team_name
    join_room(team_name)
...
def on_join_team(data):
    ...
    if not team_name or team_name not in active_teams:
        emit('error', {'message': 'Team not found or invalid team name'}); return
    team_info = active_teams[team_name]
    if team_info.get('participant2_sid'):
...
def on_leave_team(data):
    sid = request.sid
    if sid not in participant_to_team: emit('error', {'message': 'You are not in a team.'}); return
    team_name = participant_to_team[sid]
    team_info = active_teams.get(team_name)
    if not team_info: del participant_to_team[sid]; emit('error', {'message': 'Team info not found, you have been removed.'}); return
    ...
    elif team_info.get('participant2_sid') == sid: # Partner leaving
        team_info['participant2_sid'] = None
...
def start_new_round_for_pair(team_name):
    ...
    emit_dashboard_team_update() # Update dashboard with new round number for the team

@socketio.on('submit_answer')
def on_submit_answer(data):
    sid = request.sid
    if sid not in participant_to_team: emit('error', {'message': 'You are not in a team or session expired.'}); return
    team_name = participant_to_team[sid]
    team_info = active_teams.get(team_name)
    ...
    answer_for_dash = {
        ...
        'team_name': team_name, # Direct from team_name variable
    ...
    emit_dashboard_full_update() # To update total answer count
...
def get_dashboard_data():
    # This is a REST endpoint for the calculation script to fetch all data if needed
    # Or specific data based on query parameters
    ...
    # Could also include team data, round data etc.
