...
This module contains the statistics calculation functions including
uncertainty calculations, optimized computation helpers, and
mode-specific statistical calculations.
"""
...
from .cache_system import selective_cache, _classic_stats_cache, _new_stats_cache, MIN_STD_DEV
...
def _calculate_success_statistics_from_data(success_result: Tuple) -> Dict[str, Optional[float]]:
    ...
    try:
        ...
        # Use the same logic as _calculate_success_statistics but with pre-computed data
        # Calculate success statistics with uncertainties using ufloat
        # Replace trace_average_statistic with overall_success_rate
        if overall_success_rate >= 0:
            # Calculate uncertainty based on total number of rounds
